...
void BlockNode::initBlackHole()
{
	...
	// 添加粒子效果
	// 添加粒子爆炸效果
	auto pParticleRatio = ParticleSystemQuad::create("res/BB_Bomb/particle/bb_blackhole.plist");
	pParticleRatio->setPosition(_contentSize * 0.5f);
	auto particleRatioRect = pParticleRatio->getBoundingBox();
	// 获取屏幕缩放比
	auto fScalingRatio = GameDeploy::getInstance()->getScalingRatio();
	pParticleRatio->setScale(1 / fScalingRatio);
	pParticleRatio->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	addChild(pParticleRatio, 2);
...
void BlockNode::setLastBlockPosition(const cocos2d::Vec2& pos)
{
	m_LastPosition = pos;
}

const cocos2d::Vec2& BlockNode::getLastBlockPosition()
{
