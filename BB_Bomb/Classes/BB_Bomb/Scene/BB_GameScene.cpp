...
void BB_GameScene::initHomeLayer()
{
	...
	// 添加背景图片
	auto pBackgroundSprite = Sprite::create("res/BB_Bomb/Image/home/bb_home_background.png");
	auto backgroundRect = pBackgroundSprite->getBoundingBox();
	// 获取背景图的尺寸
	auto backgroundSize = pBackgroundSprite->getBoundingBox();
	...
	// 创建存放最高分的Sprite
	auto pS9HisoryScoreSprite = ui::Scale9Sprite::create("res/BB_Bomb/Image/home/bb_score_background.png");
	// 计算Label间的间距
	...
	pS9HisoryScoreSprite->setAnchorPoint(Vec2::ANCHOR_MIDDLE_TOP);
	pS9HisoryScoreSprite->setPosition(backgroundRect.size.width * 0.5f, backgroundRect.size.height * 0.34032f);
...
void BB_GameScene::initGameLayer()
{
	...
	m_pScoreLabel->setPosition(gameBackgroundRect.size.width * 0.562f, gameBackgroundRect.size.height * 0.956f);
	...
	// 添加子弹数量Label
	m_pBombNumLabel->setPosition(interfaceSize.width * 0.22f, interfaceSize.height * 0.956f);
...
void BB_GameScene::handleGameOver()
{
	...
	pScoreLabel->setPosition(settlementSize.width * 0.5f, settlementSize.height * 0.54f);
	...
	// 对历史最高分进行缩放
	while (HighScoreLabelRect.size.width + HighScoreLabelRect.size.height > interfaceSize.width * 0.552f)
	{
	...
	pHistorHighScoreLabel->setPosition(settlementSize.width * 0.5f, settlementSize.height * 0.401f);
	...
	pHomeMenu->setPosition(settlementSize.width * 0.5f, settlementSize.height * 0.177f);
...
void BB_GameScene::handleScaleScoreLabel()
{
	...
	while (scoreRect.size.width + scoreRect.size.height > interfaceSize.width * 0.2148f)
	{
		...
		// 子弹也缩小
		m_pBombNumLabel->setScale(m_fScoreLabelScale);
	}
}

void BB_GameScene::addHomeBackground()
{
	auto pGameDeploy = GameDeploy::getInstance();
	// 获取屏幕有效尺寸
	auto interfaceSize = pGameDeploy->getInterfaceSize();
	...
	// 添加名字
	auto pNameSprite = Sprite::create("res/BB_Bomb/Image/home/bb_name.png");
	pNameSprite->setAnchorPoint(Vec2::ANCHOR_MIDDLE_TOP);
	pNameSprite->setPosition(interfaceSize.width * 0.5f, interfaceSize.height * 0.96457f);
	m_pHomeLayer->addChild(pNameSprite);
	// 添加蓝色块块
	auto pBlueBlock = Sprite::create("res/BB_Bomb/Image/home/bb_blue_block.png");
	pBlueBlock->setAnchorPoint(Vec2::ANCHOR_TOP_RIGHT);
	pBlueBlock->setPosition(interfaceSize.width, interfaceSize.height * 0.8757f);
	m_pHomeLayer->addChild(pBlueBlock);
	// 添加紫色块块
	auto pPurpleBlock = Sprite::create("res/BB_Bomb/Image/home/bb_purple_block.png");
	pPurpleBlock->setAnchorPoint(Vec2::ANCHOR_TOP_LEFT);
	pPurpleBlock->setPosition(0, interfaceSize.height * 0.477f);
	m_pHomeLayer->addChild(pPurpleBlock);
	// 添加人物
	auto pPeopleSprite = Sprite::create("res/BB_Bomb/Image/home/bb_people.png");
	pPeopleSprite->setAnchorPoint(Vec2::ANCHOR_MIDDLE_TOP);
	pPeopleSprite->setPosition(interfaceSize.width * 0.5f, interfaceSize.height * 0.7984f);
	m_pHomeLayer->addChild(pPeopleSprite);
	// 添加火球
	auto pFireballSpriet = Sprite::create("res/BB_Bomb/Image/home/bb_fireball.png");
	pFireballSpriet->setAnchorPoint(Vec2::ANCHOR_BOTTOM_RIGHT);
	pFireballSpriet->setPosition(interfaceSize.width, interfaceSize.height * 0.2068f);
	m_pHomeLayer->addChild(pFireballSpriet);
	// 添加开始按钮
	auto pStartButton = MenuItemImage::create("res/BB_Bomb/Image/home/bb_strat_button.png", "res/BB_Bomb/Image/home/bb_strat_button.png",
		CC_CALLBACK_1(BB_GameScene::menuStartCallBack, this));
	auto pNode1 = pStartButton->getNormalImage();
	auto nodeSize = pNode1->getContentSize();
	auto pNode2 = pStartButton->getSelectedImage();
	pNode2->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	pNode2->setScale(1.05f);
	pNode2->setPosition(nodeSize * 0.5f);
	auto pStartMenu = Menu::create(pStartButton, nullptr);
	pStartMenu->setContentSize(Size(0, 0));
	pStartMenu->setPosition(interfaceSize.width * 0.5f, interfaceSize.height * 0.2218f - nodeSize.height * 0.5f);
	m_pHomeLayer->addChild(pStartMenu);

	// 添加帮助按钮
	auto pHelpButton = MenuItemImage::create("res/BB_Bomb/Image/home/bb_help_button.png", "res/BB_Bomb/Image/home/bb_help_button.png",
		CC_CALLBACK_1(BB_GameScene::menuHelpCallBack, this));
	pNode1 = pHelpButton->getNormalImage();
	nodeSize = pNode1->getContentSize();
	pNode2 = pHelpButton->getSelectedImage();
	pNode2->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	pNode2->setScale(1.05f);
	pNode2->setPosition(nodeSize * 0.5f);
	auto pHelpMenu = Menu::create(pHelpButton, nullptr);
	pHelpMenu->setContentSize(Size(0, 0));
	pHelpMenu->setPosition(interfaceSize.width * 0.03764f + nodeSize.width * 0.5f, interfaceSize.height * 0.10728f - nodeSize.height * 0.5f);
	m_pHomeLayer->addChild(pHelpMenu);

	// 添加排行按钮
	auto pRankingButton = MenuItemImage::create("res/BB_Bomb/Image/home/bb_ranking_button.png", "res/BB_Bomb/Image/home/bb_ranking_button.png",
		CC_CALLBACK_1(BB_GameScene::menuRankingCallBack, this));
	pNode1 = pRankingButton->getNormalImage();
	nodeSize = pNode1->getContentSize();
	pNode2 = pRankingButton->getSelectedImage();
	pNode2->setAnchorPoint(Vec2::ANCHOR_MIDDLE);
	pNode2->setScale(1.05f);
	pNode2->setPosition(nodeSize * 0.5f);
	auto pRankingMenu = Menu::create(pRankingButton, nullptr);
	pRankingMenu->setContentSize(Size(0, 0));
	pRankingMenu->setPosition(interfaceSize.width * 0.9641f - nodeSize.width * 0.5f, interfaceSize.height * 0.10728f * 0.5f);
	m_pHomeLayer->addChild(pRankingMenu);



}

void BB_GameScene::menuRankingCallBack(cocos2d::Ref* spender)
{

}

void BB_GameScene::menuHelpCallBack(cocos2d::Ref* spender)
{
